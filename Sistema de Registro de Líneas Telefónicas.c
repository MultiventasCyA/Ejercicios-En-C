...
struct Linea { //creamos structura de la linea
    int id;
    char curp[19];
    char nombre[100];
    char telefono[11];
...
struct Linea lineas[MAX]; //creamos un arreglo hasta 100 lineas telefonicas
int totalLineas = 0;
int idAutoincremental = 1;

void registrarLinea(); //funciones que usaremos luego, minetras las declaramos
void buscarLinea();
void eliminarLinea();
...
//funcion para regitrar lineas nuevas
void registrarLinea() {

    if(totalLineas >= MAX) { //si mis lineas registradas son igual o mayor a 100, no se podra registrar otra.
    ...
    do {
		...
		switch(opcionOperadora) { //menu para elegir que operadora desea registrar
		    case 1:
...
void buscarLinea() {
    ...
    if(criterio == 1) {
        ...
        for(i = 0; i < totalLineas; i++) {
            if(lineas[i].id == id) {			//compara la id en la busqueda
        ...
        for(i = 0; i < totalLineas; i++) {
            if(strcmp(lineas[i].curp, curp) == 0) { //compara cadenas de caracteres en est caso es la CURP. si lo encuentra imprime datos.
                printf("\nLinea encontrada:\n");
                printf("ID: %d\n", lineas[i].id);
                printf("Nombre: %s\n", lineas[i].nombre);
                printf("Telefono: %s\n", lineas[i].telefono);
...
void eliminarLinea() {
    ...
    for(i = 0; i < totalLineas; i++) {
        if(strcmp(lineas[i].curp, curp) == 0) { //compara cadenas de caracteres en est caso es la CURP
