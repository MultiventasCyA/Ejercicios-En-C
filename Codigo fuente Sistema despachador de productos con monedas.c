...
int main()
{
	...
	do
	{ // para repetir el proceso completo hasta que el usuario decida salir.
		while (1)
		{
			printf("==================================\n");
			...
			switch (opcion)
			{ // para validar la opci�n del producto.
			case 1:
				precio = 8;
				break;
			case 2:
			...
			break; // rompe el while(1) cuando la opci�n es v�lida
		} // fin del while

		// inicia el proceso de validacion de monedas
		total = 0; // se inicializa en 0
		while (total < precio)
		{ // mientras mi total de monedas ingreadas es menor al precio de producto, sigue solicitanto monedas.
			printf("Ingrese una moneda ($1, $2, $5, $10): ");
			scanf("%d", &moneda);
			if (moneda == 1 || moneda == 2 || moneda == 5 || moneda == 10)
			{																					 // validamos si las monedas ingresadas son de la denominacion requerida
