...
int main() {
	...
	const int DIAS_ENTREGA = 3; // 72 hrs son 3 días
	...
	scanf(" %[^\n]", nombreCliente); //scanf %s - solo guarda una cadena  y las demas la deja en el buffer de entrada
									 //" %[^\n]" significa: lee todo hasta encontrar un salto de línea (Enter)
	...
	struct tm fechaIngreso = {0}; /*es una estructura que descompone una fecha en componentes: año, mes, día, hora, minutos, segundos.
	...
	/* La función "mktime" revisa la estructura. Al detectar que el día es 33, esta misma funcion recalcula la fecha: ajusta el mes, 
	el día sobrante y, si es necesario, el año.*/
	time_t tiempo = mktime(&fechaIngreso);
	tiempo += DIAS_ENTREGA * 24 * 3600; // sumar 3 días en segundos (3 dias = 259,200 segundos).
