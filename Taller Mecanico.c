...
int main()
{
	...
	//! es una estructura que descompone una fecha en componentes: año, mes, dia, hora, minutos, segundos. El 0 es para limpiar desde el inicio todos los campos de la estructura
	struct tm fechaIngreso = {0};
	...
	//! La funcion "mktime" revisa la estructura. Al detectar que el dia es 33, esta misma funcion recalcula la fecha: ajusta el mes,	el dia sobrante y, si es necesario, el año.
	time_t tiempo = mktime(&fechaIngreso);
	tiempo += DIAS_ENTREGA * 24 * 3600; //! sumar 3 dias en segundos (3 dias = 259,200 segundos).
